...
$$
\TrSS = \sum_{i=1}^{k} n_i ( \overline{x}_i - \overline{x})^2 \,, \qquad
\ESS = \sum_{i=1}^k \sum_{j=1}^{n_i} (x_{ij} - \overline{x}_i)^2
$$
...
The F-statistic for the one-way ANOVA test follows an $F$-distribution:
...
1. **Calculation**: Compute total number of samples, samples mean and grand mean 
$$
n = \sum_{i=1}^k n_i \,, \qquad \overline{x}_i = \frac{1}{n_i} \sum_{j=1}^{n_i} x_{ij} \,, \qquad 
