...
# Lecture 8: <br> Practical Regression {background-color="#cc0164" visibility="uncounted"}
...
Dataset with $33$ entries for Gold and Stock price pairs
:::
...
data <- read.table("datasets/gold_stock.txt", header = T)

#Add column labels to data
colnames(data) <- c("Gold Price","Stock Price")
...
- The file can be downloaded here [gold_stock.txt](datasets/gold_stock.txt)

::: {.column width="54%"}

- The text file looks like this

![](images/gold_stock.png){width=73%}
...
- Remarks:
    * There is a Header
    * 1st column lists *Gold Price*
    * 2nd column lists *Stock Price*
...
To read ``gold_stock.txt`` into R proceed as follows:

1. Download [gold_stock.txt](datasets/gold_stock.txt) and move file to Desktop
...
3. Read ``gold_stock.txt`` into R and store it in data-frame ``prices`` with code
...
- We now store Gold and Stock prices in 2 vectors
    * Gold prices are in 1st column of ``prices``
    * Stock prices are in 2nd column of ``prices``

```r
gold.price <- prices[ , 1]
stock.price <- prices[ , 2]
```
...
## Plot Gold Price vs Stock Price {.smaller}
...
- Generally: the higher the value of $R^2$ the better the model
    * Textbook examples often have high values
    ...
    R^2 \geq 0.7
    $$
    * **Example:** In the *Unemployment* example of Lecture 7 we found
    $$
    R^2 = 0.8655401
    $$
...
**Example:** In the *Gold Price* Vs *Stock Price* example we have
$$
R^2 = 0.395325
$$ 
...
- The MLEs are found under ``estimate``
    * ``(Intercept)`` refers to the coefficient $\hat \alpha \qquad \implies  \qquad  \hat \alpha = 4.21285$
    * ``stock.price`` refers to the coefficient $\hat \beta \qquad \implies  \qquad  \hat \beta = - 0.06409$
...
- We fit a simple linear model to *Gold Price* Vs *Stock Price*

- We obtained the regression line

$$
\Expect[Y | x] = \hat \alpha + \hat \beta x =  4.21285 -  0.06409 \times x
$$
...
- $\hat \sigma^2$ was obtained from maximization of the likelihood function (Lecture 7) 
...
## Worked Example: Gold and Stock prices {.smaller}

- Recall that 
    * $X =$ Stock Price
    * $Y =$ Gold Price

- **Goal:** Test if *Stock Price* affects *Gold Price* at level $0.05$
...
| $\texttt{***}$, $\, \texttt{**}$, $\, \texttt{*}$, $\, \texttt{.}$   | Significance (how large is p-value): More stars are better |
: {tbl-colwidths="[30,70]"}
...
$$
\hat \beta = -0.06409 \,, \qquad \ese = 0.01424 \, , \qquad t = - 4.502 \,, \qquad 
p = 8.9 \times 10^{-5} \
$$


- The $t$-statistic computed by R can also be computed using the definition:

$$
  t   = \frac{\hat \beta - 0}{ \ese }
      = \frac{-0.06409}{0.01424} = - 4.500702 \approx - 4.502
$$
...
- The best estimate for $\beta$ is the MLE $\hat \beta = -0.06409$
...
$$
\ese =\xi_{jj}^{1/2} \,  S \,, \qquad S = \sqrt{\frac{\RSS}{n-p}}
$$
...
\RSS = \sum_{i=1}^n (y_i - \hat{y}_i)^2 \,, \qquad  S^2 := \frac{\RSS}{n-p}
$$
...
$$
\hat \alpha = 4.21285 \,, \qquad \ese = 0.32351 \, , \qquad t = 13.022 \,, \qquad 
p = 4.14 \times 10^{-14} \
$$
...
- The p-value computed by R is $p = 4.14 \times 10^{-14}$
...
$$
|t| = 13.022 > 2.040 = t_{31}(0.025) \quad \implies \quad  p < 0.05
$$
...
- The best estimate for $\alpha$ is the MLE $\hat \alpha = 4.21285$
...
We could also compute F-statistic by using the definition:
...
- Similarly, the p-value could be computed by using the definition:
